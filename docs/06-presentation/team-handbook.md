...
## D. The Research Gap & Academic Context

When presenting, never say "nobody has ever studied trust in AI agents." That is scientifically incorrect. Computational trust, distributed reputation, and tool invocation have 20+ years of mature research:
- **Classical MAS Trust (REGRET 2002, Beta 2002, FIRE 2006, TRAVOS 2006):** Solved Bayesian rating math, but designed for scalar ratings in e-commerce; blind to semantic task embeddings and LLM tool outputs.
- **Agent Identity & Registries (ERC-8004 2025, Xiong et al. 2026):** Provided open EVM discovery, but empirical measurement proves 85%–97% of endpoints are offline and 59%–90% of ratings are coordinated Sybils.
- **Context-Aware Reputation (AgentReputation / Chishti et al. 2026):** Solved competence transfer using reputation cards, but operates at the marketplace level and assumes participants consume public cards.
- **Agent Provenance (PROV-AGENT / Souza et al. 2025):** Standardized W3C execution telemetry, but as an observability tool rather than a predictive selection engine.
- **Output Verification (Huang et al. 2024, CRITIC / Gou et al. 2024):** Proved LLMs cannot self-correct without tools, but tested only single-agent reasoning rather than multi-agent service selection.

**Our Narrow, Defensible Gap:**  
Synthesizing semantic task matching (RAG), deterministic output verification, and tamper-evident provenance into a **closed-loop service selection engine**, and evaluating in a **controlled benchmark** whether local verified evidence beats reputation-only selection under adversarial conditions.

---

## E. The Central Research Question
...
## X. Academic Defense Cheat Sheet: Citing Real Literature to Judges

Every team member should memorize these exact, source-backed answers to the top 6 academic questions:

### 1. "Which papers support your problem?"
> *"Our problem is empirically proven by **Xiong et al. (June 2026, arXiv:2606.26028)**, who performed the first longitudinal measurement of the ERC-8004 agent ecosystem across Ethereum, BSC, and Base. They found that only 3% to 15% of registered agents actually expose working endpoints, and between 59.2% and 90.6% of feedback is generated by coordinated Sybils. This is supported theoretically by **Douceur’s seminal 2002 work on Sybil Attacks (IPTPS 2001)** and **Resnick et al. (CACM 2000)** on cheap pseudonyms."*

### 2. "Which paper is closest to your work?"
> *"The closest contemporary work is **AgentReputation by Chishti, Oyinloye, and Li (ACM FSE 2026 / arXiv:2605.00073)**. They similarly recognized that competence does not transfer across task contexts and proposed 3-tier decentralized reputation cards. Another close system is **PROV-AGENT by Souza et al. (IEEE eScience 2025)**, which pioneered W3C provenance capture for agent workflows."*

### 3. "What exactly is different from AgentReputation?"
> *"AgentReputation operates at the marketplace infrastructure level, publishing context-conditioned reputation cards to a shared ledger. Our framework solves the **adversarial client-agent decision problem**: when 60%–90% of registry reviewers are collusive Sybils (as Xiong et al. proved), client agents cannot rely on public cards alone. We maintain private, locally verified interaction evidence and run an evidence-grounded selection gate. Furthermore, our project conducts a controlled comparative benchmark measuring the exact performance delta between reputation cards and local evidence."*

### 4. "Why do you need evidence if reputation already exists?"
> *"Reputation is an ungrounded social aggregate—anyone can leave a rating. Classical trust literature (**REGRET by Sabater & Sierra 2002** and **FIRE by Huynh et al. 2006**) proved that heterogeneous evidence channels outperform single reputation scores. More critically, as **Hoffman et al. (ACM Computing Surveys 2009)** proved, reputation systems are fundamentally vulnerable to ballot stuffing and on-off oscillation. Grounded evidence records the exact deterministic verification outcome of past interactions, making trust an objective observation rather than a subjective opinion."*

### 5. "Why blockchain? Isn't that buzzword compliance?"
> *"We do NOT use blockchain to compute trust or run LLM inference. That would be slow and expensive. As **Zhu et al. (2026, arXiv:2608.04626)** established in their 5-layer trust taxonomy, blockchain's proper role is an immutable state anchor for decentralized identity (DIDs) and tamper-evident audit trails. Following **TrustRAG (Liu et al. 2026)**, we compute trust off-chain at the client layer and post SHA-256 state root commitments to the ledger for non-repudiable provenance."*

### 6. "Why do you need deterministic verification? Can't the LLM just self-correct?"
> *"No. That has been disproven. **Huang et al. (ICLR 2024)** proved empirically that LLMs cannot reliably self-correct reasoning errors through internal reflection alone; asking an LLM to double-check itself often causes performance to degrade. Conversely, **Gou et al. (CRITIC, ICLR 2024)** proved that tool-interactive external verification (compilers, schema validators, assertion suites) dramatically improves correctness. That is why we engineered a deterministic 4-tier verification engine."*
